...
namespace BlazorLogin.Client.Extensiones
{
    public class AutenticacionExtension : AuthenticationStateProvider
    {
        ...
        public async Task ActualizarEstadoAutenticacion(SesionDTO? sesionUsuario)
        {
            ...
            if(sesionUsuario != null)
            {
                claimsPrincipal = new ClaimsPrincipal(new ClaimsIdentity(new List<Claim>
                {
                    ...
                    new Claim(ClaimTypes.Locality,sesionUsuario.Region)
                },"JwtAuth"));
        ...
        public async Task<string> ObtenerLocalidadAsync()
        {
            var estadoAutenticacion = await GetAuthenticationStateAsync();
            return estadoAutenticacion.User.FindFirst(ClaimTypes.Locality)?.Value ?? "Sin Localidad";
        }
    }
}
